## V67 원주교구 성지·순례지 마커 표시

- 원주교구 님의 길 지도에 성지·순례지 공통 마커 9곳을 추가했습니다.
- 원동 주교좌성당, 강원 감영, 대안리 공소, 배론 성지, 서지 마을, 성 남종삼 요한·남상교 아우구스티노 유택지(묘재), 성내동 성당, 용소막 성당, 풍수원 성당을 표시합니다.
- 성지 마커는 빨간 위치핀과 흰 십자가로 표시하며, 코스 진행용 스탬프와 별도 데이터로 관리해 다음 지점 안내와 진행률 계산에 영향을 주지 않습니다.
...
## V66 전주교구 요안루갈다길
- 초남이 성지 → 치명자산 성지, 표시 거리 22km
- 업로드 GPX 실제 경로선 적용
- 성지 마커 6곳: 초남이 성지, 전주 숲정이 성지, 서천교 순교 터, 초록바위 순교 터, 전동 성당, 치명자산 성지
