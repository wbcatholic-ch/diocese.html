(() => {
  ...
  const SEOUL_FALLBACK = { lat: 36.02, lng: 128.50 };
  const ON_ROUTE_M = 45;
  ...
  function renderRouteList() {
    ...
    state.routes.forEach((route) => {
      ...
      btn.innerHTML = `
        ...
        <div class="route-foot"><span>${route.stamps?.length || 0}개 지점</span><strong>약 ${totalKm.toFixed(1)}km</strong></div>
      `;
  ...
  function openRoute(route) {
    ...
    $('map-subtitle').textContent = `${route.startName || '출발지'} → ${route.finishName || '도착지'}`;
    $('status-label').textContent = '순례길 준비 완료';
    $('status-message').textContent = '내 위치 또는 따라가기를 누르면 경로와의 거리를 계산합니다.';
  ...
  function updateRouteStatus(coords) {
    ...
    if (nearest.distanceM <= ON_ROUTE_M) {
      ...
      $('status-message').textContent = 'GPX 경로를 잘 따라가고 있습니다.';
      setChip('on', '정상');
    } else if (nearest.distanceM <= NEAR_ROUTE_M) {
      $('status-label').textContent = '경로 근처입니다';
      $('status-message').textContent = '조금 벗어났을 수 있습니다. 지도 경로선을 확인하세요.';
      setChip('near', '근처');
    } else {
      $('status-label').textContent = '경로에서 벗어났습니다';
      $('status-message').textContent = '가까운 GPX 경로선으로 돌아오세요. 도심 구간은 안내 문구를 함께 확인하세요.';
      setChip('off', '이탈');
    }
  }
